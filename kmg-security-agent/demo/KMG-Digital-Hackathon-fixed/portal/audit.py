...
def db_connected(sender, connection, **kwargs):
    record('db_connect', f'database:{connection.alias}', vendor=connection.vendor)

def db_schema_changed(sender, app_config, **kwargs):
    record('db_schema_change', f'app:{app_config.label}')
