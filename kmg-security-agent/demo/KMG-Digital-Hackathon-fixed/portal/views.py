...
def bulk_status(request):
    ...
    with transaction.atomic():
        affected = sorted(selected.values_list('pk', flat=True))
        selected.update(status=status)
        transaction.on_commit(lambda: audit.record('bulk_status','tickets',status=status,ids=affected))
    return redirect('helpdesk:list')
...
def token_issue(request):
    return JsonResponse({'token':signing.dumps({'uid':request.user.pk,'stamp':token_stamp(request.user)},salt='support-api'), 'expires_in':TOKEN_TTL})

TOKEN_TTL = 900

def token_stamp(user):
    # Changes on logout-independent credential events: password, role, queue permissions.
    perms = ','.join(sorted(user.user_permissions.values_list('codename', flat=True)))
    return signing.Signer(salt='support-api-stamp').signature(f'{user.pk}:{user.password}:{user.role}:{perms}')

def token_user(request):
    header = request.headers.get('Authorization','')
    if not header.startswith('Bearer '): raise signing.BadSignature('Missing token')
    data = signing.loads(header[7:],salt='support-api',max_age=TOKEN_TTL)
    user = User.objects.get(pk=data['uid'],is_active=True)
    if data.get('stamp') != token_stamp(user): raise signing.BadSignature('Token revoked')
    return user
...
def catalog_api(request):
    rows = list(access.tickets(request.user))
...
def queue_api(request, queue_id):
    ...
    queue.title = title
    queue.save()
    audit.record('queue_rename',f'queue:{queue.pk}')
